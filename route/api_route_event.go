...
import (
	...
	"github.com/tigerinus/xin/route/adapter/in"
	"github.com/tigerinus/xin/route/adapter/out"
	"go.uber.org/zap"
)
...
func (r *APIRoute) SubscribeEvent(c echo.Context, sourceID codegen.SourceID, params codegen.SubscribeEventParams) error {
	...
	go func(conn net.Conn, channel chan model.Event, eventNames []string) {
		...
		defer func(eventNames []string) {
			for _, name := range eventNames {
				if err := r.services.EventService.Unsubscribe(sourceID, name, channel); err != nil {
					logger.Printf("error when trying to unsubscribe an event type", zap.Error(err), zap.String("source_id", sourceID), zap.String("name", name))
				}
			}
		}(eventNames)
		...
		for {
			...
			if err != nil {
				logger.Printf("error when trying to marshal event", zap.Error(err))
				continue
			}
